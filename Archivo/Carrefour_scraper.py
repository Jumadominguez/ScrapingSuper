...
# Encabezados CSV
encabezados = ["ID_Producto", "Nombre", "Precio", "Fecha", "Fuente"]
productos = []
...
for i, card in enumerate(cards, start=1):
    try:
        ...
        entero1 = card.find_elements(By.CLASS_NAME, clases_precio[2])[0].text.strip()
        grupo = card.find_element(By.CLASS_NAME, clases_precio[3]).text.strip()
        entero2 = card.find_elements(By.CLASS_NAME, clases_precio[2])[1].text.strip()
        ...
        productos.append([i, nombre, precio, datetime.today().strftime('%Y-%m-%d'), "Home Carrefour"])
